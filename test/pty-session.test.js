...
test("_parseOutput ignores echoed wrapper text and keeps real output", () => {
  ...
  const raw = [
    `echo "${preMarker}"; echo hi; echo "${cwdMarker}$(pwd)__"; echo "${marker}_$?__"`,
    preMarker,
    "hi",
...
test("write throws when session is not alive", () => {
  ...
  assert.throws(() => session.write("hello"), /is no longer alive/);
});
...
test("resize throws when session is not alive", () => {
  ...
  assert.throws(() => session.resize(120, 40), /is no longer alive/);
});
...
test("resize sends command and updates cols/rows", () => {
  ...
  const calls = [];
  session.ptydClient = {
    resize: (id, cols, rows) => calls.push({ id, cols, rows }),
  };
...
test("_formatWaitOutput returns empty for match-only mode", () => {
  const session = createSession();
  const result = session._formatWaitOutput(
    "some output",
    "match-only",
    50,
    null,
